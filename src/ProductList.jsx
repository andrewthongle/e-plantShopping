...
function ProductList() {
  ...
  const isAddedToCard = (item) => {
    return addedToCart.some((itemInCart) => itemInCart === item.name);
  };
  const totalItemsAmount = () =>
    cart.reduce((total, item) => total + item.quantity, 0);

  useEffect(() => {
    console.log("Updated addedToCart state:", cart);
  }, [cart]);
        ...
        <div style={styleObjUl}>
          ...
          <div>
            ...
            <a href="#" onClick={(e) => handleCartClick(e)} style={styleA}>
              <h1 className="cart">
                <p className="cart_quantity_count">{totalItemsAmount()}</p>
                <svg
      ...
      {!showCart ? (
        <div className="product-grid">
          {plantsArray.map((plants) => {
            return (
              <div key={plants.category} className="category">
                <div className="plantname_heading">
                  <h2 className="plant_heading">{plants.category}</h2>
                </div>
